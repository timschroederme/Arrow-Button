...
#pragma mark -
#pragma mark TSLeftButtonCell Interface

@interface TSLeftButtonCell : NSButtonCell

@end


#pragma mark -
#pragma mark TSLeftButtonCell Implementation

@implementation TSLeftButtonCell
...
-(NSBezierPath*)bezierPathForRect:(NSRect)frame
{
    ...
    [path moveToPoint:NSMakePoint(frame.origin.x+frame.size.width-roundedRadius, frame.origin.y)];
    [path lineToPoint:NSMakePoint(frame.origin.x+triangleWidth,frame.origin.y)];
    [path lineToPoint:NSMakePoint(frame.origin.x, (frame.origin.y+(frame.size.height/triangleHeightRatio)))];
    [path lineToPoint:NSMakePoint(frame.origin.x+triangleWidth, frame.origin.y+frame.size.height)];
    
    // Bottom right corner
    NSPoint c1org = NSMakePoint(frame.origin.x+frame.size.width-roundedRadius, frame.origin.y+frame.size.height);
    [path lineToPoint:c1org];
    NSPoint c1end = NSMakePoint(frame.origin.x+frame.size.width, frame.origin.y+frame.size.height-roundedRadius);
    [path curveToPoint:c1end
         controlPoint1:NSMakePoint(c1org.x+roundedRadius, c1org.y)
         controlPoint2:c1end];
    
    // Top right corner
    NSPoint c2org = NSMakePoint(frame.origin.x+frame.size.width, frame.origin.y+roundedRadius);
    [path lineToPoint:c2org];
    NSPoint c2end = NSMakePoint(frame.origin.x+frame.size.width-roundedRadius, frame.origin.y);
...
- (void)drawFocusRingMaskWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
    ...
    NSRect focusRect = [self modifyRect:NSInsetRect(cellFrame, 0.0, 1.0)
                                 deltaX:-0.5 deltaY:0.0 deltaW:0.5 deltaH:-1.0];
...
#pragma mark -
#pragma mark TSLeftButton Implementation

@implementation TSLeftButton

// Replace the default button cell with our custom class
- (id)initWithCoder:(NSCoder*)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        NSButtonCell *originalCell = [self cell];
        TSLeftButtonCell *customCell = [[TSLeftButtonCell alloc] init];
...
- (NSView *)hitTest:(NSPoint)aPoint
{
    ...
    if (x < triangleWidth) {
        ...
        float upperLimit = verticalMiddle+x;
        float lowerLimit = verticalMiddle-x;
